...
{
    class MLEProcessor : TrustPredictionMethod
    {
        ...
        public override void updateTrustFromObservation(int PredictedTag) // workes on updates
        {
            ...
            foreach (var report in _tags)
            {
                min_dist = double.PositiveInfinity;
                selected_id = -1;
                user_id = report.UserID;
                _user.MLE_SUM += Math.Pow(report.Tag - PredictedTag, 2);
                _user.NTags++;

                delta_star = Math.Sqrt(_user.MLE_SUM / (double)_user.NTags);
                ...
                _user.PredictedTrust = (int)(inputParams.possibleTrusts[selected_id] * 100);
            }
        }
    }
}
